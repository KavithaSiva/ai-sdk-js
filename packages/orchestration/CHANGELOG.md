...
- 1bd196c: [New Functionality] Introduce `buildDpiMaskingProvider()` convenience function to build masking provider `SAP Data Privacy Integration`.
- 1bd196c: [New Functionality] Support setting `metadata_params` property in `buildDocumentGroundingConfig()` convenience function.

### Patch Changes

- Updated dependencies [1bd196c]
